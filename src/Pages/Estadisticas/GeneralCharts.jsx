...
import { PieChart, PersonStanding, Baby, CloudHail } from 'lucide-react';
...
export const Charts = () => {
    ...
    const { AllAnalytics, data, response } = useData()
        ...
        if (data) {
            if (data.Data) {
                if (data.Data.length != 0)
                    setValueGen(data.Data.length)
                if (data.Seccion.length != 0) {

                    //  Porcentaje
                    const getTotal = data.Seccion.reduce((acumulador, dato) => {
                        return acumulador + parseInt(dato.lista_nominal);
                    }, 0);
                    setValueSeccion(getTotal)
                    const res = Math.trunc((valueGen / valueSeccion) * 100)
                    setPorcentaje(res)

                    // ? Edad porcentaje

                    const getYear = data.Data.map(x => x.fecha_nacimiento.slice(0, 4))
                    const date = new Date()
                    ...
                    let resultadoFrecuencia = contarRepeticiones(data.Data);
                    console.log(resultadoFrecuencia)
                    setSecSexo(resultadoFrecuencia)
                    ...
                    <Tabs defaultValue="general" className="space-y-4">
                        ...
                        <TabsContent value="general" className="space-y-4">
                            ...
                            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-7">
                                ...
                                <Card className="col-span-8">
                                    <CardHeader>
                                        <CardTitle> Secccion /  Sexo</CardTitle>
                                        <CardDescription>
                                        </CardDescription>
                                    </CardHeader>
                                    <CardContent>
                                        <SimpleBar data={secSexo} label1={'cantidad'} color={'#24c35b'} />
                                    </CardContent>
                                </Card>
                            </div>
                        </TabsContent>
                        <TabsContent value="seccion" className="space-y-4">
                            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                                <Card>
                                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                                        <CardTitle className="text-sm font-medium">
                                            Promovidos vs lista nominal
                                        </CardTitle>
                                        <PieChart />
                                    </CardHeader>
                                    <CardContent>
                                        <div className="text-2xl font-bold"> Total 389</div>
                                        <p className="text-xs text-muted-foreground">
                                            45.1% de la lista nominal
                                        </p>
                                    </CardContent>
                                </Card>
                                <Card>
                                    ...
                                    <CardContent>
                                        <div className="text-2xl font-bold"> 18-65 años </div>
                                        <p className="text-xs text-muted-foreground">
                                            45% de promovidos tiene edad de 35
                                        </p>
                                    </CardContent>
                                </Card>
                                <Card className="col-span-2">
                                    ...
                                    <CardContent>
                                        <div className="text-2xl font-bold"> Hombres 239 y Mujeres 150 </div>
                                        <p className="text-xs text-muted-foreground">
